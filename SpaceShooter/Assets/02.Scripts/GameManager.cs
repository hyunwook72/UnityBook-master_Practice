...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //points = spawnPointGroup?.GetComponentsInChildren<Transform>(); //points 변수를 일반 배열로 설정했을 때
        //spawnPointGroup?.GetComponentsInChildren<Transform>(points); //points 변수를 List로 설정(Parent에 있는 컴포넌트도 추출됨)
        foreach(Transform point in spawnPointGroup)
        {
            points.Add(point);
        }

        //위 코드를 이해하기 편하게 일일이 다 풀어서 작성했을 때의 코드
    ...
    void CreateMonsterPool()
    {
        for(int i=0; i<maxMonsters; i++)
        {
            ...
            //_monster.name = "Monster_" + i.Tostring("00"); //ToString메서드를 사용한 방법
